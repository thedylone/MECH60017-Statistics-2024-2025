...
plot(points, gamma_pdf,
  type = "l",
  lwd = 2,
  col = "blue",
  xlab = "x",
  ylab = "Probability",
  main = paste("Gamma distribution (k =", shape, ", θ =", scale, ")")
)

gamma_cdf <- pgamma(points, shape = shape, scale = scale)
plot(points, gamma_cdf,
  type = "s",
  lwd = 2,
  col = "blue",
  xlab = "x",
...
generate_distributions <- function(n, lambda, shape, scale) {
  # Poisson distribution
  poisson_rvs <- rpois(n, lambda = lambda)

  # Gamma distribution
  gamma_rvs <- rgamma(n, shape = shape, scale = scale)

  return(list(poisson_rvs = poisson_rvs, gamma_rvs = gamma_rvs))
}

distributions <- generate_distributions(n, lambda, shape, scale)
poisson_rvs <- distributions$poisson_rvs
gamma_rvs <- distributions$gamma_rvs
```
...
plot_distributions <- function(distributions, points, poisson_pmf, gamma_pdf) {
  n <- length(distributions$poisson_rvs)
  poisson_rvs <- distributions$poisson_rvs
  gamma_rvs <- distributions$gamma_rvs
  ...
  hist(poisson_rvs,
    ...
    xlab = "x",
    ylab = "Frequency",
    main = paste("Poisson distribution (n =", n, ")")
  )
  # overlay true probability density function
  lines(points, poisson_pmf,
    lwd = 2,
    col = "orange"
  )
  ...
  hist(gamma_rvs,
    ...
    xlab = "x",
    ylab = "Frequency",
    main = paste("Gamma distribution (n =", n, ")")
  )
  # overlay true probability density function
  lines(points, gamma_pdf,
    lwd = 2,
    col = "orange"
  )
}

plot_distributions(distributions, points, poisson_pmf, gamma_pdf)
```
...
# Poisson distribution
poisson_mean <- lambda
poisson_variance <- lambda
print(paste("Poisson distribution: mean =", poisson_mean))
print(paste("Poisson distribution: variance =", poisson_variance))

# Gamma distribution
gamma_mean <- shape * scale
gamma_variance <- shape * scale^2
print(paste("Gamma distribution: mean =", gamma_mean))
print(paste("Gamma distribution: variance =", gamma_variance))
```
...
```{r sample-moments}
# Poisson distribution
poisson_mean_sample <- mean(poisson_rvs)
poisson_variance_sample <- var(poisson_rvs)
print(paste("Poisson distribution: sample mean =", poisson_mean_sample))
print(paste("Poisson distribution: sample variance =", poisson_variance_sample))

# Gamma distribution
gamma_mean_sample <- mean(gamma_rvs)
gamma_variance_sample <- var(gamma_rvs)
print(paste("Gamma distribution: sample mean =", gamma_mean_sample))
print(paste("Gamma distribution: sample variance =", gamma_variance_sample))
```
...
for (n in n_values) {
  ...
  print(paste("Poisson distribution (n =", n, "): sample mean =", mean(poisson_rvs)))
  print(paste("Poisson distribution (n =", n, "): sample variance =", var(poisson_rvs)))

  print(paste("Gamma distribution (n =", n, "): sample mean =", mean(gamma_rvs)))
  print(paste("Gamma distribution (n =", n, "): sample variance =", var(gamma_rvs)))
}
```